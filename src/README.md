...
| `api.ts`, `auth.ts` | API requests, record types, current-user lookup, and role-based **My space** destination. |
...
The UI redirects users according to their role, but the backend is responsible for permission checks. The dashboard currently shows account details; customer self-service subscriptions are not implemented.
